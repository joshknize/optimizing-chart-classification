...
def main():  
    ...
    if len(sys.argv) > 1:
        ...
        raise ValueError("Please provide an argument for a json config location. E.g. `python ViT.py ./data/config.json`")
    ...
    attn_maps = []
    if cfg['output']['error_analysis'] in ["attention_overlay", "all_attention_overlay"]:
